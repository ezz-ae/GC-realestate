...
export function LeadsTable({ leads, isAdmin, teamMembers = [] }: LeadsTableProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 rounded-2xl border border-border/60 bg-card p-4 shadow-sm md:grid-cols-[1.2fr,0.4fr,0.4fr]">
        <Input
            ...
            >
              ...
              <div className="col-span-2 flex items-center justify-end gap-2">
                ...
                {isAdmin ? (
                  <Button
                    size="sm"
                    className="h-8 gold-gradient px-3 text-[10px] font-bold shadow-sm"
                    disabled={loading === `assign:${lead.id}` || !(assignments[lead.id] ?? lead.assigned_broker_id)}
                    onClick={() => handleAssign(lead)}
                  >
